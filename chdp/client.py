from discord import AutoShardedClient, Color, Status, Game, Status, Embed, TextChannel, Role, Member, Reaction, Message

from chdp.chdp_funcs import *
from chdp.errors import AsyncTimeoutError
from chdp.handler import Handler
...
class CHDPClient(AutoShardedClient):
    ...
    def run_bot(self, **kwargs):
        self.starttime = time() # 업타임 계산을 위해
    ...
    async def use_cmd(self, message) -> bool:
        ...
        try: args = message.content.split(index)[1][self.ix:].split(' ')
        except: args = []
    ...
    def check_permission(self, author, p) -> bool:
        ...
        if p in ['guildowner', 'owner', 'serverowner']: return bool(author.guild.owner == author)
